 ...
 */
 /*
  * Support of release_call functions added by Alberto Díez
 */
#include <time.h>
...
void free_p_dialog(p_dialog *d)
{
	...
	tmb.free_dlg(d->dialog_s);
	tmb.free_dlg(d->dialog_c);
	shm_free(d);
}
		...
		d_lock(i);
			...
			while(d){
				...
				if (d->expires<=d_time_now) {
					...
					if (!terminate_p_dialog(d)) del_p_dialog(d);

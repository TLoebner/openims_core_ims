/**
 * $Id: acctstatemachine.c 139 2007-02-13 20:27:02Z placido $
 *  
...
char *acc_states[]={"Idle","PendingS","PendingE","PendingB","Open","PendingI","PendingL"};
/** Strings for the accounting client events */
char *acc_events[]={"AccEv_Start","AccEv_Event","AccEv_BufferedStart","AccEv_Rcv_Suc_ACA_Start","AccEv_SndFail","AccEv_Rcv_Failed_ACA_Start","AccEv_Stop",
 ...
 * @param msg - if a Diameter message was received or should be sent this is it, or NULL if not
...
int acct_cli_sm_process(acc_session *s,acc_event_t event,AAAMessage *msg, int session_locked)
{
	...
	LOG(L_INFO,"DBG:acct_cli_sm_process(): AccountingSession %.*s \tState %s \tEvent %s\n",
		s->sID->len,s->sID->s,acc_states[s->state],acc_events[event-101]);
		...
		case ACC_ST_IDLE:
			switch (event){
				...
				case ACC_EV_BUFFEREDSTART:
					...
					break;
				default:
					LOG(L_DBG,"DBG:acct_cli_sm_process(): In state %s invalid event %s\n",
						acc_states[s->state],acc_events[event-101]);
					goto error;
			}
			break;		

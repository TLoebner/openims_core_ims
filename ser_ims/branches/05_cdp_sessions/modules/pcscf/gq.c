...
str modify_call_id(str call_id, int tag)
{
	str* t;
...
AAAMessage *Gq_AAR(struct sip_msg *req, struct sip_msg *res, int tag)
{
	...
	AAAAuthSession* auth = cdpb.AAACreateAuthSession(pdf_peer, 
						modify_call_id(call_id, tag), SESSION_STATE_MAINTAINED); 
	if (!auth) goto error;												 
...
AAAMessage* Gq_STR(struct sip_msg* msg, int tag)
{
	...
	AAAAuthSession* auth = cdpb.AAAGetAuthSession(modify_call_id(call_id, tag));
	if (!auth) goto error;
